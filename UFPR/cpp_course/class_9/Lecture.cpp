...
void Lecture::addContent(std::string description, unsigned short workloadContent) {
	this->contents.push_back(new Content{description, workloadContent});
}

void Lecture::removeContentById(unsigned int id) {
	std::list<Content*>::iterator it{this->contents.begin()};

	for (; it != this->contents.end(); ++it) {
		if ((*it)->getId() == id) {
			delete *it;
...
std::list<Content*>& Lecture::getContents() {
	return contents;
}